...
def get_np_argmax(image_path, model_path):
    ...
    x = preprocess_input(np.expand_dims(keras.utils.img_to_array(
        keras.utils.load_img(image_path, target_size=(224, 224))), axis=0))
...
class InferenceServicer(inference_pb2_grpc.InferenceServicer):
    def Predict(self, request, context):
        ...
        return inference_pb2.Prediction(value=int(prediction), confidence=int(confidence))
